...
- [conjur_secret](docs/data-sources/secret.md) (also available as [ephemeral resource](docs/ephemeral-resources/secret.md))
- [conjur_certificate_issue](docs/data-sources/certificate_issue.md) (requires Secrets Manager Saas with Certificate Manager integration)
- [conjur_certificate_sign](docs/data-sources/certificate_sign.md) (requires Secrets Manager Saas with Certificate Manager integration)

The provider can also manage the following Secrets Manager resources:
- [conjur_authenticator](docs/resources/authenticator.md)
- [conjur_policy_branch](docs/resources/policy_branch.md)
- [conjur_host](docs/resources/host.md) (SaaS only)
- [conjur_group](docs/resources/group.md)
- [conjur_secret](docs/resources/secret.md) (SaaS only)
- [conjur_membership](docs/resources/membership.md)
- [conjur_permission](docs/resources/permission.md)
- [conjur_swa_trust_domain](docs/resources/swa_trust_domain.md) (SaaS only)
- [conjur_swa_server_group](docs/resources/swa_server_group.md) (SaaS only)
- [conjur_swa_server](docs/resources/swa_server.md) (SaaS only)
- [conjur_swa_node_group](docs/resources/swa_node_group.md) (SaaS only)
...
### Using `conjur login` credentials
The provider can also reuse credentials cached by the [Conjur CLI](https://github.com/cyberark/conjur-cli-go).
Authenticate once with the CLI:
```sh
conjur init saas --url "$SM_URL"
conjur login -i "$CONJUR_AUTHN_LOGIN"
```
Verify the session:
```sh
conjur whoami
```
`conjur init` writes `~/.conjurrc` and `conjur login` caches the needed credentials; the provider reads
both automatically, so the provider block can be left empty:
```terraform
terraform {
  required_providers {
    conjur = {
...
provider "conjur" {}
```
This is convenient for local, interactive use. For CI/automation, prefer explicit provider attributes or
environment variables so the run does not depend on a pre-authenticated CLI session on the runner.

### Using cloud or certificate authentication
Instead of a static `login`/`api_key` pair, the provider can authenticate as a workload using its cloud
identity or an X.509 client certificate, by setting `authn_type` to one of `aws`, `azure`, `gcp`, `jwt`, or
`cert`.
...
- `authn_cert` (String, Sensitive) Inline PEM content of the client certificate for authn-cert mTLS authentication. Use authn_cert_file to specify a file path instead
- `authn_cert_file` (String, Sensitive) Path to the PEM-encoded client certificate file for authn-cert mTLS authentication
- `authn_cert_key` (String, Sensitive) Inline PEM content of the private key for the authn-cert client certificate. Use authn_cert_key_file to specify a file path instead
- `authn_cert_key_file` (String, Sensitive) Path to the PEM-encoded private key file for the authn-cert client certificate
...
**Note:** The `conjur_secret` data source is also available as an [ephemeral resource](docs/ephemeral-resources/secret.md) (`ephemeral "conjur_secret"`).
